...
public class SipLayer implements SipListener {
	...
	public SipLayer(String username, String ip, int port)
		...
		properties.setProperty("javax.sip.STACK_NAME", "TextClientStack");
		properties.setProperty("javax.sip.IP_ADDRESS", ip);

		// DEBUGGING: Information will go to files

		properties.setProperty("gov.nist.javax.sip.TRACE_LEVEL", "36");
	...
	public void processResponse(ResponseEvent evt) {
		...
		if ((status >= 200) && (status < 300)) { // Success!
			String msg = new String(response.getRawContent());
			String mark = msg.substring(0, msg.indexOf(":"));
			if (mark.equals("CLIENTLIST")) {
				String list = msg.substring(msg.indexOf(":") + 1);
				messageProcessor.processInfo(list);
				return;
			}else if(mark.equals("REBACKWHISPER")){
				messageProcessor.processInfo("WHISPER");
				return;
			}else if(mark.equals("REBACKGROUP")){
				messageProcessor.processInfo("GROUP");
				return;
			}
		}
	...
	public void processRequest(RequestEvent evt) {
		...
		String mark = message.substring(0, message.indexOf(":"));
		
		if(mark.equals("CLIENTLIST")){
			String list = message.substring(message.indexOf(":") + 1);
			messageProcessor.processInfo(list);
		}else if(mark.equals("WHISPER")){
			String from_name = message.substring(message.indexOf(":")+1, message.indexOf("@"));
			String msg = message.substring(message.indexOf("@") + 1);
			
			//FromHeader from = (FromHeader) req.getHeader("From");
			messageProcessor.processWhisperMessage(from_name,
					msg);
			...
			try { // Reply with OK
				...
				response.setContent("", contentTypeHeader);
			...
			//FromHeader from = (FromHeader) req.getHeader("From");
			messageProcessor.processGroupMessage(from_name,
					msg);
			Response response = null;
			try { // Reply with OK
				response = messageFactory.createResponse(200, req);
